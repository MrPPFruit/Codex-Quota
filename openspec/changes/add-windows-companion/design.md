...
当前产品由 Swift 6.2、AppKit 和 SwiftUI 构成，额度模型与 app-server 协议语义已经稳定，但所有窗口、进程、签名、登录项和生命周期代码均为 macOS 专用。Windows 官方桌面宿主采用 MSIX/WindowsApps；locator 必须从正在运行的官方包身份推导系统保护包根，不能用 PATH、进程名或用户可写镜像猜测执行位置。
...
- 当前用户 Windows ChatGPT/Codex 版本的 package publisher、helper signer 与 WindowsApps 包内相对路径，需要真实设备 probe 确认。
