...
- **THEN** 系统背景采样位于 WPF 内容之后，内部色场保持半透明且延伸到同一窗口边界；若材质初始化失败，则使用不透明中性浅色内容表面降级，并记录脱敏原因，不得恢复为多圈描边或深色霓虹卡片
...
- **THEN** 系统以 PerMonitorV2 处理 DIP、物理像素与窗口区域转换，窗口始终位于当前显示器工作区且文字、透明角、系统材质和命中区域保持正确
...
- **THEN** 验收记录分别覆盖真实额度、Codex 启停、系统 Acrylic 背景采样、圆形/圆角区域、焦点、hover/拖动、浅色/深色/高频桌面背景、100%/125%/150%/200% DPI、空闲动画 CPU、SmartScreen、睡眠恢复、登录启动和进程清理；未执行项不得标记通过，离屏 WPF PNG 不得替代桌面合成证据
