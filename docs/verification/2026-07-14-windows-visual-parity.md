...
- 当前公开版本为唯一的 GitHub Prerelease：[v0.1.0](https://github.com/MrPPFruit/Codex-Quota/releases/tag/v0.1.0)。其 Windows 资产来自通过本文件记录的验收构建；后续修复覆盖该统一 Release，不再创建独立 preview 发布页。
- Windows x64 ZIP SHA-256：`c64a9a6bbba27e4314c422e3dc22da0df4bf14daf7662abac49642c6ba0d03f8`。
- Release 创建后已重新下载 ZIP 与 `.sha256` 文件，通过 `shasum -a 256 -c`，归档只包含 `CodexQuota.exe` 和 `README-Windows.txt`。
