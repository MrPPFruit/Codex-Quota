...
- Windows 版使用原生 WPF/.NET 10，不依赖 Electron；登录后驻留托盘，仅在官方 Codex 运行时显示气泡。
...
Windows 预览版要求 Windows 11 x64 和 Microsoft Store 安装的官方 Codex 桌面应用。它是 self-contained 单文件程序，不要求用户另装 .NET；Windows 10、ARM64 与 WSL 暂不支持。
...
3. Codex 打开时气泡显示，Codex 退出时气泡隐藏。托盘菜单可关闭登录启动或退出。
